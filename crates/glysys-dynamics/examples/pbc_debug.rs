...
fn main() {
    ...
    let options = BuildOptions { add_water: true, add_ions: false, padding_angstrom: 9.0, ..Default::default() };
    let system = SystemBuilder::new(options).unwrap().prepare_pdb_str(pdb).unwrap();
    ...
    let protocol = SimulationProtocol {
        ...
        equilibration_steps: 0, production_steps: 200, save_every: 200,
        minimization_iterations: 200, timestep_fs: 1.0, friction_per_ps: 0.0,
        constraints: ConstraintModel::Settle, cutoff_angstrom: Some(9.0), seed: 11,
        ..Default::default()
    };
    let mut sim = ExplicitSimulation::new(&system, protocol).unwrap();
    for _ in 0..200 { sim.step().unwrap(); }
    ...
    let boxv = BoxVectors::new(sim.state.box_angstrom[0], sim.state.box_angstrom[1], sim.state.box_angstrom[2]).unwrap();
    let w: Vec<_> = sim.state.coordinates.iter().map(|p| boxv.wrap(*p)).collect();
    let pairs = PbcNeighborList::build(&w, &boxv, 9.0, 1.5).unwrap();
    let e = field.evaluate(&sim.state.coordinates, &boxv, &pairs.pairs, &rf, 9.0).unwrap();
    println!("virial_terms bond/angle/tors/pair: {:.1?}", e.virial_terms);
    println!("pair virial LJ/elec: {:.1?}", e.virial_pair_split);
    let c = e.components;
    println!("E bond={:.1} angle={:.1} tors={:.1} vdw={:.1} elec={:.1} total={:.1}",
        c.bonds, c.angles, c.proper_torsions + c.improper_torsions,
        c.van_der_waals, c.electrostatics, c.total());
    println!("virial total: {:.1}, PE: {:.1}", e.virial, e.components.total());
